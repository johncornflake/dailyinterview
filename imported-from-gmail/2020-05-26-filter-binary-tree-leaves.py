...
k
, filter the binary tree such that its leaves don't contain the value 
k
.  Here are the rules:
- If a leaf node has a value of 
k
, remove it.
- If a parent node has a value of 
k
...
f
"value: {self.value}, left: ({self.left.__repr__()}), right: ({self.right.__repr__()})"
def
